...
flmenu= Menu(hh)
menu1= Menu(flmenu,tearoff=False)
...
class typing:
    ...
    def __init__(self):
        ...
        self.clock=[]   
        self.string_all=open(file='文章.txt',mode='r')#读取文章
    ...
    def open_file(self):
        self.file_path=tkinter.filedialog.askopenfilename(title=u'选择文件',initialdir=(),filetypes=[('文本文档','.txt')])
        print(self.file_path)
        self.string_all=open(file=self.file_path,mode='r')#读取文章
        print(self.string_all)
        self.b_all=self.strQ2B(self.string_all.read())#将输入文章字符变更为半角
        self.all=''.join(self.b_all.splitlines())#read返回字符串 .splitlines()去除换行
...
menu1.add_command(label='选择例文',command=tp.open_file)
menu1.add_command(label='退出',command=hh.quit)
flmenu.add_cascade(label='设置',menu=menu1)#cascade级联
...
#hh.iconbitmap('favicon.ico')
hh.config(menu=flmenu)
